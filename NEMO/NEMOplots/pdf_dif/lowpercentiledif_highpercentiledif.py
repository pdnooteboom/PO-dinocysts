...
#barmax = 10; barmin = barmax*-1;barrange = range(barmin,barmax+1,4);
...
plt.contourf(longitudes,latitudes,was)
plt.show()
...
if(plotcontour):
    #plt.contour(xs,ys,nzs,contourrange, colors='k',zorder=2)
#    plt.contour(scipy.ndimage.zoom(xs, zoomam),scipy.ndimage.zoom(ys, zoomam),scipy.ndimage.zoom(nzs, zoomam),contourrange, colors='k',zorder=2)    
    plt.contour(xs,ys,ndimage.gaussian_filter(nzs, sigma=sigma, order=0),contourrange, colors='k',zorder=2)
#%%  nth percentile var2

pf = np.load(dirRead +  'box-percentile/TM_box-mean%dpercentiledif_'%(n)+var2+'_ddeg%d_sp%d_dd%d'%(ddeg, int(sp),int(dd))+'.npz')
...
if(plotcontour):
#    plt.contour(xs,ys,nzs,contourrange2, colors='k',zorder=2)
#    plt.contour(scipy.ndimage.zoom(xs, zoomam),scipy.ndimage.zoom(ys, zoomam),scipy.ndimage.zoom(nzs, zoomam),contourrange2, colors='k',zorder=2)
...
x = xs[~zs.mask]#[0].ravel()
y = ys[~zs.mask]#[0].ravel()
