...
class TM_ParticleSet(object):
    ...
    def __init__(self,lon,lat, ddeg, Lons, Lats):
        print 'Total number of particles: ', len(lon)

        print lon.shape

        where_lonlat = np.where(np.logical_and(lat[:,0]<np.max(Lats)+ddeg, lat[:,0]>=np.min(Lats)))

        lat = lat[where_lonlat[0],:]
        lon = lon[where_lonlat[0],:]
    ...
    def setup_TM(self,ddeg,Lons,Lats):
        ...
        print 'shape:  ',self.TMindex_I.shape,'    and [0]:', self.TMindex_I[0]
        ...
        maxlat = np.amax(self.Lats)
        minlat = np.amin(self.Lats)
        for i in range(len(self.lons_I)):
            if not np.isnan(self.lons_I[i]) or ma.is_masked(self.lons_I[i]):            
#                if(self.TMindex_I[i]<len(self.N_initial_I)): 
                if self.lats_I[i] <= maxlat and self.lats_I[i] >= minlat: 
...
#            print 'jops'
#            print self.lons_I[i], self.lats_I[i]
#            print i
 #           print self.TMindex_I.shape
 #           print self.N_initial_I.shape
 #           print self.TMindex_I[i]
