...
sp = 6. #The sinkspeed at the surface  (m/day)
#Let the sinking speed increase linearly to:
spd = 45. #(m/day) 
#At depth:
increasedepth = 100. # (m)
spconstantdepth = 2000. #(m)

dd = 10. #The dwelling depth

directory = ''
dirwrite = '/OFES/OFESres/particledata/spincrease_scenario/' #Directory to write the data
...
landfilename = directory + "topography_OFES.nc"
...
#print 'Depth at initial locations: ',dep
...
def Sink(particle, fieldset, time, dt):
    if(particle.depth>fieldset.dwellingdepth):
        if(particle.depth<=fieldset.sp_depth1):
            sp_depth = fieldset.sinkspeed
        elif(particle.depth<=fieldset.sp_depth2):
            sp_depth = fieldset.sinkspeed + ((fieldset.sinkspeed2-fieldset.sinkspeed)/(fieldset.sp_depth2-fieldset.sp_depth1))*(particle.depth-fieldset.sp_depth1)
        else:
            sp_depth = fieldset.sinkspeed2
        particle.depth = particle.depth + sp_depth * dt
    if(particle.depth<fieldset.dwellingdepth):
...
outfile = "grid_id"+str(posidx) +'_dd'+str(int(dd)) +'_spincrease'+"_res"+str(res)
